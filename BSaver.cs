...
namespace TimeTableManager.IO {
    ...
    public class BSaver {
        ...
        /// <summary>セーブメイン
        /// </summary>
        public void Save(string file, BTimeTable ret) {
            ...
            #endregion
            #region デフォルトの人員配置
            if (ret.DefaultRequire != null && ret.DefaultRequire != BRequirePatterns.NULL) {
            ...
            BRequirePatterns work = null;
            work = ret.GetDefaultRequire(System.DayOfWeek.Monday);
            if (work != null && work != BRequirePatterns.NULL) {
        ...
        /// <summary>勤務シフトのセーブ
        /// </summary>
        public XmlElement SavePatterns (XmlDocument doc, BTimeTable all) {
            ...
            for (int i = 0; i < all.Patterns.Size(true); i++) {
                BPattern pattern = all.Patterns[i, true];
                if (!pattern.BuiltIn) {
        ...
        /// <summary>休日のセーブ
        /// </summary>
        public XmlElement SaveDayOffs (XmlDocument doc, BTimeTable all) {
            XmlElement element = doc.CreateElement("dayoffs");
            SaveTmElement(doc, element, all.DayOffs);
            for (int i = 0; i < all.DayOffs.Size(); i++) {
                BDayOff dayoff = all.DayOffs[i];
                XmlElement pNode = doc.CreateElement("dayoff");
                SaveTmElement(doc, pNode, dayoff);
                pNode.SetAttribute("name", dayoff.Name);
                pNode.SetAttribute("start", dayoff.StartDate.ToString(BSaver.DATEFORMAT));
                pNode.SetAttribute("end", dayoff.EndDate.ToString(BSaver.DATEFORMAT));
                element.AppendChild(pNode);
            }
            return element;
        }
        ...
        public XmlElement SaveMembers (XmlDocument doc, BTimeTable all) {
            ...
            for (int i = 0; i < all.Members.Size(true); i++) {
                ...
                for (int j = 0; j < member.PatternSize; j++) {
                    BPattern pattern = member[j];
                    XmlElement nNode = doc.CreateElement("pattern");
                    nNode.SetAttribute("id", pattern.ObjectID.ToString());
                    pNode.AppendChild(nNode);
                }
                #region 稼働日
                pNode.SetAttribute("avl-monday", member.IsAvailableDay(BTimeTable.tMonday).ToString());
                pNode.SetAttribute("avl-tuesday", member.IsAvailableDay(BTimeTable.tTuesday).ToString());
                pNode.SetAttribute("avl-wednesday", member.IsAvailableDay(BTimeTable.tWednesday).ToString());
                pNode.SetAttribute("avl-thursday", member.IsAvailableDay(BTimeTable.tThursday).ToString());
                pNode.SetAttribute("avl-friday", member.IsAvailableDay(BTimeTable.tFriday).ToString());
                pNode.SetAttribute("avl-saturday", member.IsAvailableDay(BTimeTable.tSaturday).ToString());
                pNode.SetAttribute("avl-sunday", member.IsAvailableDay(BTimeTable.tSunday).ToString());
                #endregion
                #region 連続稼働日数と空白時間
        ...
        /// <summary>人員配置のセーブ
        /// </summary>
        public XmlElement SaveRequirePatterns (XmlDocument doc, BTimeTable all) {
            ...
            for (int i = 0; i < all.Requires.Size(true); i++) {
                BRequirePatterns require = all.Requires[i, true];
                if (!require.BuiltIn) {
                    ...
                    for (int j = 0; j < require.Size(); j++) {
                        BPattern pattern = require.GetPattern(j);
        ...
        /// <summary>日付のセーブ
        /// </summary>
        public void SaveScheduledDate (XmlDocument doc, XmlElement rNode, BTimeTable all) {
            for (int i = 0; i < all.Size(); i++) {
                bool edited = false;
                BScheduledDate sdate = all[i];
                XmlElement pNode = doc.CreateElement("scheduleddate");
                SaveTmElement(doc, pNode, sdate);
                // 日付がもっとも大事？
                pNode.SetAttribute("date", sdate.Date.ToString(BSaver.DATEFORMAT));
                // 人員配置
                if (sdate.Require != null && sdate.Require != BRequirePatterns.NULL) {
                ...
                for (int j = 0; j < sdate.ValidMemberSize; j++) {
                    BSchedule schedule = sdate[j];
                    XmlElement nNode = doc.CreateElement("schedule");
                    SaveTmElement(doc, nNode, schedule);
                    if ((schedule.Member != null && schedule.Member != BMember.NULL && schedule.Pattern != null && schedule.Pattern != BPattern.NULL) || (schedule.Notes.Trim() != "")) {
        ...
        public void SaveTmElement (XmlDocument doc, XmlElement element, ITimeTableElement obj) {
            ...
            element.SetAttribute("created", obj.Created.ToString(BSaver.DATEFORMAT));
            if (obj.Removed != null) {
                element.SetAttribute("removed", ((DateTime)obj.Removed).ToString(BSaver.DATEFORMAT));
            }
            // プロパティ
